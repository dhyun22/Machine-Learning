    ...
    def finalize(self):
        ...
        for q in self.questions:
          ...
          if self.points[q] == self.maxes[q]:
              passed = "passed"
          scores[q] = {'name': q, 'score': self.points[q], 'max_score': self.maxes[q], 'status': passed}
        print('------------------')
...
""")
        json_file = {'tests': [scores[x] for x in self.questions]}
        directory_path = "/autograder/results"
        os.makedirs(directory_path, exist_ok=True)
        file_path = os.path.join(directory_path, "results.json")
        json_string = json.dumps(json_file, indent=2)
